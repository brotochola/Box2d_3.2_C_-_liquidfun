...
function createPhysicsApi(Module) {
  ...
  const destroyParticleGroup = wrap("destroy_particle_group", null, ["number"]);
  const setParticleSubSteps = wrap("set_particle_sub_steps", null, ["number"]);
  ...
  class PhysicsWorld {
    ...
    createParticleSystem(options = {}) {
      const o = options || {};
      const ok = createParticleSystem(
        this.worldId,
        o.radius ?? 0.045,
        o.density ?? 1.0,
        o.maxParticles ?? 4096,
        o.strictContactCheck ? 1 : 0,
      );
      if (!ok) {
        throw new Error("createParticleSystem failed");
      }
      if (o.subSteps != null) {
        setParticleSubSteps(o.subSteps);
      }
      return this;
    }
    ...
    createParticleGroupBox(options = {}) {
      const o = options || {};
    ...
    createParticleGroupCircle(options = {}) {
      const o = options || {};
    ...
    getReadyPayload() {
      ...
      return {
        ...
        particleCapacity: getParticleCapacity(),
        particleRadius: getParticleRadius(),
        particleCountByteOffset: getParticleCountByteOffset(),
